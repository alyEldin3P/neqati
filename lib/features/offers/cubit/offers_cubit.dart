  ...
  Future<void> loadOffers() async {
    ...
    try {
      ...
      for (var data in offersData) {
        log(
          '   📄 Raw Offer: ${data['title']} (ID: ${data['id']}) - is_active: ${data['is_active']}, end_date: ${data['end_date']}',
        );
      }
      ...
      final offers =
          offersData.map((data) {
            final offer = Offer(
              ...
              startDate:
                  data['start_date'] != null &&
                          data['start_date'].toString().isNotEmpty
                      ? DateTime.parse(data['start_date'])
                      : DateTime.now(),
              endDate:
                  data['end_date'] != null &&
                          data['end_date'].toString().isNotEmpty
            ...
            log(
              '   🎁 Mapped Offer: ${offer.title} (ID: ${offer.id}) - isActive: ${offer.isActive}, endDate: ${offer.endDate}',
            );
            return offer;
          }).toList();
      ...
      log(
        '🔍 Active offers after client-side filtering: ${activeOffers.length}',
      );

      if (activeOffers.length != offers.length) {
        log(
          '⚠️ WARNING: Found inactive offers in supposedly active offers list!',
        );
        final inactiveOffers =
            offers.where((offer) => !offer.isActive).toList();
        for (var inactiveOffer in inactiveOffers) {
          log(
            '   ❌ Inactive Offer Found: ${inactiveOffer.title} (ID: ${inactiveOffer.id}) - isActive: ${inactiveOffer.isActive}',
          );
        }
      }
